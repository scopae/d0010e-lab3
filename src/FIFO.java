...
public class FIFO implements Queue {
	...
	private Vector <Object> fifoQueue = new Vector <Object>();
	...
	//adds an item to the queue and checks the size of the queue against the max size
	...
	/**
	 * 	Removes the first object from the queue. If the queue is empty, a NoSuchElementException is thrown. 
	*/		
	...
	// throws an exception if there's no object at index 0, else it returns the object
	public Object first() {
	...
	public boolean isEmpty() {
		if (fifoQueue.size() == 0) {
			...
			//true if list is empty
				...
				return false;
				// false if list contains elements
			}
	}
	
	// checks if the list contains the same objects in the same order
	public boolean equals(Object f) {
		// tests if they are instances of the same type of object else throws an exception
		...
		{
			//iterates over the objects and logically checks if they hold the same value at the same index
			
			for (int i = 0; i < fifoQueue.size(); i++) {
			
				//checks first if the first object isn't null while the second one is, then checks if the first object is null while the second isn't
	...
	// iterates over the array and adds the element at index i to a string and returns it
